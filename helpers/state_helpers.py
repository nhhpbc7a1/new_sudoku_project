...
def pencil_action(self):
    if (self.is_using_pencil):
        self.btnPencil.configure(fg_color="#F4CE98");
    else:
        self.btnPencil.configure(fg_color="#1F509A");
...
def fill_resolve_AI(self,board):
    ...
    for row in range(self.game_size**2):
        for col in range(self.game_size**2):
            if (board[row][col] <= 0):
                is_solved = False
            if (self.robot1_start_state[row][col] == 0 and board[row][col] > 0):
                self.robot1_entries[int_to_alpha(row) + int_to_alpha(col)].delete("all")
                self.robot1_entries[int_to_alpha(row) + int_to_alpha(col)].create_text(15, 15, text=str(board[row][col]), font=("Arial", 20), anchor="center", fill="green")
    
    self.done_game_action("AI " + self.algorithm_name, is_solved)
...
def done_game_action(self, winner, is_solved):
    ...
    if (is_solved == True):
        ...
        messagebox.showinfo(winner, "Không thể tìm thấy lời giải" +  f" \n Thời gian hoàn thành: {minutes:02}:{seconds:02}")
...
def start_game_action(self, algorithm_name):
    ...
    if algorithm_name == "logic_based":
            self.logic_based(board)
            
    if algorithm_name == "hybrid":
        self.hybrid_sudoku_solver(board)            
